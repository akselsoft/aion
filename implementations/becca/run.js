...
async function runEngine(projectRoot) {
    try {
        ...
        console.log(`🔧 [free] Loaded project config from ${projectRoot}/config.json`);

        // Step 2: Load sources
        console.log(`📂 [free] Loading sources from ${projectRoot}`);
        const sources = await loadSourceAdapters(projectRoot, config.sources || []);
        console.log(`📚 [free] Loaded ${sources.length} source(s)`);
        ...
        console.log(`✅ [free] Engine pipeline completed`);
        ...
        console.error(`❌ [free] Error running pipeline:`, err.message);
        throw err;
    }
}
