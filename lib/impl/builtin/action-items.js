...
module.exports = {
  async run(ctx, engineCfg, personaCfg) {
    ...
    const keywords = normalizeKeywords(engineCfg.keywords || engineCfg.keywordTerms || DEFAULT_KEYWORDS);
    const now = new Date();

    const existing = includeExisting ? parseExisting(outputFile) : [];
    const discovered = discoverItems(ctx.passedFiles || [], inputType, now, keywords);
...
function discoverItems(items, inputType, now, keywords = DEFAULT_KEYWORDS) {
  ...
  for (const item of selected) {
    for (const doc of item.documents || []) {
      ...
      for (const line of lines) {
        ...
        if (!looksActionable(cleaned, keywords)) continue;
        out.push({
          status: 'Open',
          item: cleaned,
...
function looksActionable(line, keywords = DEFAULT_KEYWORDS) {
  if (!line || line.length < 8) return false;
  if (/^(done|completed|cancelled|canceled)\b/i.test(line)) return false;
  return buildKeywordRegex(keywords).test(line);
}
...
function buildKeywordRegex(keywords) {
  const terms = normalizeKeywords(keywords).map(term => {
    if (term.startsWith('/') && term.lastIndexOf('/') > 0) {
      return term.slice(1, term.lastIndexOf('/'));
    }
    return escapeRegex(term).replace(/\\\s+/g, '\\s+');
  });
  if (!terms.length) return buildKeywordRegex(DEFAULT_KEYWORDS);
