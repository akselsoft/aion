...
module.exports = {
  async run(ctx, engineCfg, personaCfg) {
    ...
    const basePrompt = typeof engineCfg.prompt === 'string' ? engineCfg.prompt : (engineCfg.prompt?.base || '');
    const promptMode = (engineCfg.promptMode || engineCfg.prompt?.mode || 'replace').toLowerCase();
    const nowMs = Date.now();
    const cutoffMs = computeCutoff(nowMs, projectRoot, engineCfg);
    ...
    if (hasWildcard(baseDir)) {
      ...
      if (engineCfg.sinceLastRun) writeLastRun(projectRoot, nowMs);
      return;
    }
...
function computeCutoff(nowMs, projectRoot, engineCfg) {
  let cutoff = null;
...
function listGlobDocs(globPath, cutoffMs, engineCfg = {}) {
  ...
  const sortMode = String(engineCfg.sort || engineCfg.sortBy || 'modified-asc').toLowerCase();
...
function compareCandidates(a, b, sortMode) {
  if (sortMode === 'modified-desc' || sortMode === 'mtime-desc' || sortMode === 'newest') {
    return b.lastModified - a.lastModified || a.name.localeCompare(b.name);
  }
  if (sortMode === 'name-desc') return b.name.localeCompare(a.name);
  if (sortMode === 'name-asc' || sortMode === 'name') return a.name.localeCompare(b.name);
  return a.lastModified - b.lastModified || a.name.localeCompare(b.name);
}
...
function readLastRun(projectRoot) {
  ...
  try {
    ...
    const ts = Date.parse(data.lastRun);
...
function writeLastRun(projectRoot, nowMs) {
  const p = path.join(projectRoot, '.artifacts-last-run.json');
  try {
    fs.writeFileSync(p, JSON.stringify({ lastRun: new Date(nowMs).toISOString() }, null, 2), 'utf8');
  } catch (err) {
