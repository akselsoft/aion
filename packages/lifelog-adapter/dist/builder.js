...
export async function runPipeline(cfg, win) {
    ...
    let items = [];
    do {
        const params = { limit: cfg.limit, direction: cfg.direction };
        if (cursor)
            params.cursor = cursor;
        else {
            params.start = win.startISO;
            params.end = win.endISO;
        }
    ...
    if (cfg.outputFormat === "ndjson") {
        ...
        for (const p of files) {
            ...
            const lifelog = (json?.data?.lifelog ?? json);
            if (!lifelog?.id) {
                malformed++;
                continue;
            }
            if (!lifelogWithinWindow(lifelog, win))
                continue;
        ...
        if (malformed > 0)
            console.warn(`[warn] skipped malformed/corrupt files: ${malformed}`);
        return;
    }
    // Markdown mode
    const byDay = new Map();
    ...
    for (const day of days) {
        ...
        if (!md.trim())
            continue; // no sections survived → nothing to write
