import { LifelogHeader } from "./types.js";


type ContentItem = {
    content?: string | null;
    type?: string | null;         // "blockquote" | "heading1" | "heading2" | ...
    speakerName?: string | null;  // "Speaker 1" etc
...
function parseWorkHours(s?: string | null): { start: number; end: number } | null {
    if (!s) return null;
...
function minutesUTC(iso?: string | null): number | null {
    if (!iso) return null;
    const d = new Date(iso);
    return d.getUTCHours() * 60 + d.getUTCMinutes();
}
function tokenize(s: string) { return s.toLowerCase().match(/[a-z0-9@#._-]+/g) ?? []; }
function hasNumber(s: string) { return /\d/.test(s); }
function questionDensity(lines: string[]) { return lines.filter(x => x.includes("?")).length / Math.max(1, lines.length); }

type ScoreResult = { score: number; reasons: string[]; trivial: boolean };

function scoreSection(
    title: string,
    convoKept: ContentItem[],
    durationSec: number,
    ...
    if (dur > 0.15) score += dur * 0.35; else score += dur * 0.20;
    ...
    if (uniqueSpeakers.length >= 2) { score += 0.10; reasons.push("≥2 speakers"); }

    // Text-based signals
    const allText = [title, ...convoKept.map(c => c.content ?? "")].join(" ");
    const toks: string[] = tokenize(allText);
    const qden = questionDensity(convoKept.map(c => c.content ?? ""));
    score += Math.min(qden * 2, 1) * 0.10;                  // many questions → more “discussion”
    if (toks.some(hasNumber)) { score += 0.05; reasons.push("numbers present"); }

    const includeKeywords = (opts?.includeKeywords ?? []) as string[];
    const ignoreKeywords = (opts?.ignoreKeywords ?? []) as string[];

    const includes = includeKeywords.filter(k => toks.includes(k));
    const ignores = ignoreKeywords.filter(k => toks.includes(k));

    if (includes.length) { score += Math.min(0.2, includes.length * 0.05); reasons.push(`keywords: ${includes.join(",")}`); }

    // Penalties
    if (ignores.length) { score -= Math.min(0.25, ignores.length * 0.06); reasons.push(`ignored: ${ignores.join(",")}`); }
    ...
    if (wh) {
        ...
        if (mins != null) {
            ...
            if (inWin) { score += 0.05; reasons.push("work hours"); }
            else { score -= 0.05; reasons.push("off hours"); }
        }
    }
...
function looksLikeJunk(s: string): boolean {
    ...
    if (wordCount(t) <= 3) {
        ...
        if (/^\*{0,2}sh?t$/.test(t) || /^damn$/.test(t)) return true;
    }
    return false;
}

function byStart(a?: string | null, b?: string | null) {
...
function fmtTime(iso?: string | null): { ymd: string; hm: string } | null {
    if (!iso) return null;
    const d = new Date(iso);
...
export function renderDayMarkdown(
    day: string,
    lifelogs: LifelogHeader[],
    stripJunk: boolean,
    opts?: {
        minLines?: number;
        minSec?: number;
        keepShortIfHasHeading?: boolean;

        // make these optional; even if you don't use all of them yet,
        // typing them here suppresses the 'never' errors.
        debug?: boolean;
        roster?: string[];
    ...
    for (const ll of sorted) {
        // headings
        const contents = (ll as any)?.contents as ContentItem[] | undefined;
        ...
        const convoKept: ContentItem[] = [];
        let removed = 0;
        for (const c of convoAll) {
            const text = (c.content ?? "").trim();
            if (!text) continue;
            if (stripJunk && looksLikeJunk(text)) { removed++; continue; }
            convoKept.push(c);
        }
        ...
        if (ll.startTime && ll.endTime) {
            ...
            if (!isNaN(ms) && ms > 0) durationSec = Math.round(ms / 1000);
        }
        ...
        for (const c of convoKept) {
            ...
            if (who && !uniqueSpeakers.includes(who)) uniqueSpeakers.push(who);
        }
        ...
        if (h1 !== title) {
            if (h1) lines.push(`**Heading1:** ${h1}`);
        }
        if (h2) lines.push(`**Heading2:** ${h2}`);
        if (h1 || h2) lines.push("");
        ...
        if (convoKept.length) {
            ...
            if (removed > 0) lines.push(`_(${removed} short acknowledgments removed)_`);
            lines.push("");
        } else {
